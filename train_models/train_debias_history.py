...
def solver_debias_history(
        opt, logger, train_dataset, val_dataset, finetune,
        image_encoder, question_encoder,
        history_encoder_fg, history_encoder_bg,
        debias_history_model_fg, debias_history_model_bg
):
    ...
    optimizer_fg = optim.Adamax(
        list(image_encoder.parameters()) + list(question_encoder.parameters()) +
        list(history_encoder_fg.parameters()) + list(debias_history_model_fg.parameters()),
        lr=initial_lr
    )
    optimizer_bg = optim.Adamax(
        list(history_encoder_bg.parameters()) + list(debias_history_model_bg.parameters()),
        lr=initial_lr
    )
...
def train_debias_history(opt, logger, train_dataloader, train_dataset, val_dataloader, val_dataset,
    ...
    optimizer_fg, optimizer_bg, scheduler_fg, scheduler_bg, iterations = solver_debias_history(
        opt=opt,
        logger=logger,
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            hist_features_fg, hist_features_mask_fg, hist_length_fg = models_dict.get('history_encoder_fg')(
                batch['cap'], batch['cap_len'], batch['hist'], batch['hist_len'])
            hist_features_bg, hist_features_mask_bg, hist_length_bg = models_dict.get('history_encoder_bg')(
                batch['cap'], batch['cap_len'], batch['hist'], batch['hist_len'])

            output_fg = models_dict.get('debias_history_model_fg')(
                batch_size, img_features, ques_features, hist_features_fg, hist_features_bg.detach(),
            ...
            output_bg = models_dict.get('debias_history_model_bg')(
                batch_size, img_features, ques_features, hist_features_fg.detach(), hist_features_bg,
                img_features_mask, ques_features_mask, hist_features_mask_fg, hist_features_mask_bg, batch
            )
