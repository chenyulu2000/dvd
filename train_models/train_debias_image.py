...
from models.models import DebiasImageModel
...
def solver_debias_image(
        opt, logger, train_dataset, val_dataset, finetune,
        image_encoder_fg, image_encoder_bg,
        question_encoder, history_encoder,
        debias_image_model_fg, debias_image_model_bg
):
    ...
    optimizer_fg = optim.Adamax(
        ...
        list(history_encoder.parameters()) + list(debias_image_model_fg.parameters()),
        lr=initial_lr
    )
    optimizer_bg = optim.Adamax(
        list(image_encoder_bg.parameters()) + list(debias_image_model_bg.parameters()),
        lr=initial_lr
    )
...
def train_debias_image(opt, logger, train_dataloader, train_dataset, val_dataloader, val_dataset,
    ...
    models_dict = {
        ...
        'history_encoder': HistoryEncoder(opt=opt, shared_word_embed=shared_word_embed),
        'debias_image_model_fg': DebiasImageModel(opt=opt, shared_word_embed=shared_word_embed),
        'debias_image_model_bg': DebiasImageModel(opt=opt, shared_word_embed=shared_word_embed)
    }

    for model_name, model in models_dict.items():
        models_dict[model_name] = DistributedDataParallel(module=model.cuda(), device_ids=[opt.local_rank])

    optimizer_fg, optimizer_bg, scheduler_fg, scheduler_bg, iterations = solver_debias_image(
        opt=opt,
        logger=logger,
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            hist_features, hist_features_mask, _ = models_dict.get('history_encoder')(
                batch['cap'], batch['cap_len'], batch['hist'], batch['hist_len']
            )
            output_fg = models_dict.get('debias_image_model_fg')(
                batch_size, img_features_fg, img_features_bg.detach(), ques_features, hist_features,
                img_features_mask_fg, img_features_mask_bg, ques_features_mask, hist_features_mask,
                batch
            )
            output_bg = models_dict.get('debias_image_model_bg')(
                batch_size, img_features_fg.detach(), img_features_bg, ques_features, hist_features,
                img_features_mask_fg, img_features_mask_bg, ques_features_mask, hist_features_mask,
                batch
            )
        ...
        if epoch >= opt.validate_epoch and opt.local_rank == 0:
            ...
            for i, batch in enumerate(tqdm(eval_dataloader)):
                ...
                with torch.no_grad():
                    ...
                    output_fg = models_dict.get('debias_image_model_fg')(
                        batch_size, img_features_fg, img_features_bg, ques_features, hist_features,
